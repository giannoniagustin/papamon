...
bool HourChooser(const char* label, tm& dateOut,const char* dateFormat )
{
    ...
    ImGui::Text("Hora");
    ImGui::PushID(1222);
    if (ImGui::SmallButton(arrowLeft)) { dateOut.tm_hour -= 1; RecalculateDateDependentFields(dateOut); }
    ImGui::SameLine();
    if (dateOut.tm_hour < 10) { ImGui::Dummy(ImVec2(arrowLeftWidth, 0)); ImGui::SameLine(); }
    ImGui::Text("%d", dateOut.tm_hour);
    ImGui::SameLine();

    if (ImGui::SmallButton(arrowRight)) { dateOut.tm_hour += 1; RecalculateDateDependentFields(dateOut); }
    ImGui::PopID();
    ImGui::Dummy(ImVec2(0, 20));
    /////////////////////////////////// MINUTES
    ImGui::Text("Minutos");
    ImGui::PushID(1234);
    if (ImGui::SmallButton(arrowLeft)) { dateOut.tm_min -= 1; RecalculateDateDependentFields(dateOut); }
    ImGui::SameLine();
    if (dateOut.tm_min <10 ){ ImGui::Dummy(ImVec2(arrowLeftWidth, 0)); ImGui::SameLine();}
    ImGui::Text("%d", dateOut.tm_min);
    ImGui::SameLine();
    
    if (ImGui::SmallButton(arrowRight)) { dateOut.tm_min += 1; RecalculateDateDependentFields(dateOut); }
    ImGui::PopID();
    ImGui::Dummy(ImVec2(0, 20));
    /////////////////////////////////// SECS
    ImGui::Text("Segundos");
    ImGui::PushID(1245);
    if (ImGui::SmallButton(arrowLeft)) { dateOut.tm_sec -= 1; RecalculateDateDependentFields(dateOut); }
    ImGui::SameLine();
    if (dateOut.tm_sec < 10){ ImGui::Dummy(ImVec2(arrowLeftWidth, 0)); ImGui::SameLine(); }
    ImGui::Text("%d", dateOut.tm_sec);
    ImGui::SameLine();

    if (ImGui::SmallButton(arrowRight)) { dateOut.tm_sec += 1; RecalculateDateDependentFields(dateOut); }
    ImGui::PopID();
    // return combo
    ImGui::EndCombo();
}
