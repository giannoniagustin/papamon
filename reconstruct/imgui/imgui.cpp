...
bool ImGui::BeginChildImage(const char* str_id, ImTextureID texId, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    bool created = BeginChildEx(str_id, window->GetID(str_id), size_arg, border, extra_flags);
    ImVec2 uv0(0, 0);
    ImVec2 uv1(1, 1);
    ImVec4 tint_col(1.0, 1.0, 1.0, 1.0);
    ...
    const ImVec2 totalSizeWithoutPadding(size.x + innerSpacing, size.y );
    ...
    ImRect image_bb(start, start + size_arg);
...
void ImGui::End()
{
    ...
    if (window->Active && window->DC.CurrentColumns != NULL)
    {
        EndColumns();
    }
    else
