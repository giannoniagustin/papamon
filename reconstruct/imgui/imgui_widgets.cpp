...
bool ImGui::VOwnSliderScalar(const char* label, const ImVec2& size,  float value, float p_min, float p_max)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Slider behavior
    int height = (frame_bb.Max.y - frame_bb.Min.y);
    float offset = height * p_min/(p_max-p_min);
    ...
    for (int i = 0; i < (value+1); i++)
    {
        ...
        grab_bb.Min.x = frame_bb.Min.x;
        grab_bb.Max.x = frame_bb.Max.x;
        grab_bb.Min.y = frame_bb.Max.y + offset - i*5;
        grab_bb.Max.y = frame_bb.Max.y + offset - (i+1)*5;
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 frame_size)
{
    ...
    if (values_count >= values_count_min)
    {
        ...
        /// draw middle line
        ImVec2 middlepos0, middlepos1;
        middlepos0 = ImVec2(inner_bb.Min.x, (inner_bb.Min.y + inner_bb.Max.y)/2.0f);
        middlepos1 = ImVec2(inner_bb.Max.x, (inner_bb.Min.y + inner_bb.Max.y) / 2.0f);
        window->DrawList->AddLine(middlepos0, middlepos1, GetColorU32(ImGuiCol_Button),3.0f);

        /// draw left line
        middlepos0 = ImVec2(inner_bb.Min.x, inner_bb.Min.y );
        middlepos1 = ImVec2(inner_bb.Min.x, inner_bb.Max.y);
        window->DrawList->AddLine(middlepos0, middlepos1, GetColorU32(ImGuiCol_Button), 3.0f);
