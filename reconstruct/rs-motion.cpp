...
int main(int argc, char * argv[]) try
{
    ...
    while (app)
    {
        ...
        camera.render_camera(theta);


        if (frameIndex % 1000 == 0)
        {
            float3 euler;
            euler.z = (theta.x * 180 / PI_FL);
            euler.y = -(theta.y * 180 / PI_FL);
            euler.x = -(theta.z - PI_FL / 2) * 180 / PI_FL;

            std::cout << "Pitch " << euler.x << " Yaw " << euler.y <<  " Roll " << euler.z << "\n";
            std::cout << " -----------------------------------------------------" << "\n";
        }
        frameIndex++;
    }
